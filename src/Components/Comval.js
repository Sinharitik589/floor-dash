...
//Component for showing rule values
...
const Comval = (props) => {
    ...
    const { selected_index, selectIndexes, check, all, start, end, trade, cieling, floor,
        cargo, loadingArea, dischargeArea, loadingPort, dischargePort, conflict, loadingCountry,
        dischargeCountry, pod, pol, denomination, status, floors, setFloors, index, duplicate, setDuplicate } = props;

    let [menu, toggleMenu] = useState(false);//For Changing Rule Type
    let [del, setDel] = useState(props.del);
    let [modal, showModal] = useState(false);
    let [editPerm, setEditPerm] = useState(false) //manages permisssion for a given BU
    // const DeleteElement = () => <div className="position-absolute h-100  row no-gutters align-items-center w-100" style={{ left: 0 }} >
    //     <div className="w-100 bg-secondary" style={{ height: 1, zIndex: 500 }} />
    // </div>
    ...
    const duplicateRule = () => {
        let object = { all, start, end, trade, cieling, floor, cargo, loadingArea, dischargeArea, loadingPort, dischargePort, conflict: true, loadingCountry, dischargeCountry, pod, pol, denomination, status: "DUPLICATE", floors, setFloors, index, duplicate: true };
    ...
    const renderMain = () => {
        ...
        if (editSection) {
            ...
            return (<Row noGutters className="flex-nowrap comval align-items-center " style={{ backgroundColor: (check || all) ? "#dfe9fb" : (menu) ? "#EAF1FF" : (del) ? "#eeeeee" : "" }}
                onMouseLeave={() => {
                    toggleMenu(false);
                }} onMouseEnter={() => toggleMenu(true)} >

                <Col className="p-0  position-relative " xs={1}>
                        ...
                        <Col className="p-0" xs={3}>
                            <Row noGutters>
                                ...
                                <Col className="p-0 pr-1" xs={6}>
                                    ...
                                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content " style={{ backgroundColor: (check || all) ? "#dfe9fb" : (menu) ? "#EAF1FF" : (del) ? "#eeeeee" : "" }} >
                                        <div className="col-12 p-0 pr-1 left-border">
                                            <label className=" m-0 text-secondary">{floor}</label>
                                        </div>
                        ...
                        <Col className="p-0" xs={3}>
                            ...
                            <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content " style={{ backgroundColor: (check || all) ? "#dfe9fb" : (menu) ? "#EAF1FF" : (del) ? "#eeeeee" : "" }} >
                                ...
                                <div className="col-5 p-0 pr-1 position-relative left-border">
                                    ...
                                    <label className=" m-0 text-secondary">{cieling}</label>
                                </div>
