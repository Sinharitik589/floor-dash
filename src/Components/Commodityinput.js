...
const Commodityinput = (props) => {
    ...
    let [charge, setCharge] = useState((props.charge) ? props.charge : "");
    let [comGroup, setComGroup] = useState((props.comGroup) ? props.comGroup : ""); let [start, setStart] = useState((props.start) ? reverseDate(props.start, "input") : "");
    ...
    useEffect(() => {
        ...
        if ((props.selected_index.length > 1) && (props.selected_index.findIndex(val => val === index) >= 0) && (!props.duplicate)) {
            ...
            if (fcharge !== null) setCharge(fcharge)
            if (fcomGroup !== null) setComGroup(fcomGroup)
        }
    }, [floadingArea,
        fdischargeArea,
        floadingCountry,
    ...
    const createRule = async () => {
        let { floors, setFloors, index, status, setAlarm, alarm, selected_index, selectIndexes } = props;
        let object = {
            start: (start != "") ? reverseDate(start, "output") : "", end: (end != "") ? reverseDate(end, "output") : "", trade, charge, comGroup, cargo,
        ...
        if (editSection >= 0) {
            let status = (floors[editSection].status == undefined) ? "edit" : object.status
    ...
    return (<>{
        open && <Row noGutters className="flex-nowrap input-row">
                ...
                <div className="top-option-tab row no-gutters p-1 pt-2 pb-2  align-items-center w-100 h-max-content">
                    ...
                    <div className="col-9 w-100">
                        <Dropdown onSelect={(e) => {
                            ...
                            if ((props.selected_index.findIndex(val => val === index) >= 0)) {
                ...
                <div className="top-option-tab row no-gutters p-1 pt-2 pb-2  align-items-center w-100 h-max-content">
                    <div className="col-12 p-0 pr-1">
                        <Dropdown onSelect={(e) => {
                            ...
                            if ((props.selected_index.findIndex(val => val === index) >= 0)) {
                                props.setComGroup(commodity_array[e].COMM_SUB_GROUP);
                            }
                            if (props.duplicate && props.selected_index.length > 0) {
                                let array = [...props.floors];
                                let item = array[index];
                                item.comGroup = commodity_array[e].COMM_SUB_GROUP;
                                props.setFloors(array);
                            }
                        }}>
                            <Dropdown.Toggle
                            ...
                            <Dropdown.Menu >
                                {
                                    commodity_array.map((val, index) => <Dropdown.Item key={`input_Comgroup_${index}`} title={val.TEXT} eventKey={index} active={index === loadingPort}>{val.COMM_SUB_GROUP}</Dropdown.Item>)
                                }
                            </Dropdown.Menu>
                        </Dropdown>
                    </div>
                        ...
                        <div className="top-option-tab row no-gutters p-1 pt-2 pb-2  align-items-center w-100 h-max-content">
                            <div className="col-12 p-0 pr-1">
                                <Dropdown onSelect={(e) => {
                                    ...
                                    <Dropdown.Menu >
                                        {
                                            denomination_array.map((val, index) => <Dropdown.Item key={`input_discharge_deno_${index}`} eventKey={index} active={index === denomination}>{val}</Dropdown.Item>)
                                        }
                                    </Dropdown.Menu>
                                </Dropdown>
                            </div>
                            ...
                            <div className="col-5 p-0 pr-1">
                                <input type="number" value={charge}
                                    onChange={(e) => {
                                        ...
                                        if ((props.selected_index.findIndex(val => val === index) >= 0)) {
                                            props.setCharge(parseInt(e.target.value))
                                        }
                                        if (props.duplicate && props.selected_index.length > 0) {
                                            let array = [...props.floors];
                                            let item = array[index];
                                            item.charge = parseInt(e.target.value);
                                            props.setFloors(array);
                                        }
                                    }}
