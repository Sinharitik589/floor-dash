...
const Markval = (props) => {
    const { selected_index, selectIndexes, check, all, start, end, trade, charge, cargo, loadingArea, dischargeArea,
    ...
    let [editPerm, setEditPerm] = useState(false)
    ...
    const duplicateRule = () => {
        let object = { duplicate: true, all, start, end, trade, charge, cargo, loadingArea, dischargeArea, loadingPort, dischargePort, conflict: true, loadingCountry, dischargeCountry, pod, pol, denomination, status: "DUPLICATE", floors, setFloors, index };
    ...
    const deleteRule = async () => {
        try {
            ...
            console.log(`%c ${res.data}`, 'background: #222; color: #bada55');
            if (res.data) {
            ...
            return (<Row noGutters className="flex-nowrap comval align-items-center  " style={{ backgroundColor: (check || all) ? "#dfe9fb" : (menu) ? "#EAF1FF" : (del) ? "#eeeeee" : "" }} onMouseLeave={() => {
                ...
                <Col className="p-0 pr-1 position-relative " xs={1}>
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content " style={{ backgroundColor: (check || all) ? "#dfe9fb" : (menu) ? "#EAF1FF" : (del) ? "#eeeeee" : "" }}  >
                        ...
                        <div className="col-9 p-0 left-border">
