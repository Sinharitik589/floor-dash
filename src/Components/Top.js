...
const Top = ({ selectIndexes, floors, setTempFloors, setFloors, user, currentBu, nav }) => {
    ...
    let [cieling, setCieling] = useState("");
    ...
    const sort = async (orderby) => {
        ...
        let rules = res.data.Rules.map(val => {
            let { RULE_TYPE, TRADE, POL_AREA, POL_COUNTRY, POL_GROUP_CODE, POL_CODE, POD_AREA, POD_COUNTRY, POD_GROUP_CODE, POD_CODE, CONTAINER_TYPE, START, END, STATUS, CONFLICT, ID } = val;
    ...
    //This will reset all the input for filter
    const reset = () => {
        setTrade("")
        setLoadingArea("")
        setLoadingCountry("")
    ...
    //Will look for the changes and change the rule display
    //accordingly
    ...
    const fetchFilterResult = async () => {
        ...
        if (pod !== "") params.POD_GROUP_CODE = pod;
        ...
        if (floor !== "") params.param2 = floor;
        if (floor === "") delete params.param2;
        if (cieling !== "") params.param3 = cieling;
        if (cieling === "") delete params.cieling;
        ...
        let final_params = {};
        //sending filter options only when atleast one is filled
        if (keys.length > 0) {
        ...
        try {
            ...
            setFloors(rules);//setting the display rules
    ...
    }, [loadingArea, dischargeArea, loadingCountry, dischargeCountry, loadingPort, dischargePort, pol, pod, cargo, floor, cieling, start, end, trade, denomination])
                        ...
                        <Dropdown.Menu>
                            {
                                trade_array.length > 0 && trade_array.map((val, index) => {
                                    return <Dropdown.Item title={val.TXTLG} eventKey={index} key={`Top_dropdown${index}`} active={index === trade}>{val.ZCR_TRGR}</Dropdown.Item>
                                })
                            }
                        </Dropdown.Menu>
                    </Dropdown>
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        ...
                        <div className="col-3 p-0 pr-1">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        discharge_country_array.map((val, index) => <Dropdown.Item eventKey={index} key={`top_discharge_country_${index}`} title={val.TXTKG} active={index === loadingCountry}>{val.COUNTRY}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                        <div className="col-2  p-0">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        discharge_port_array.map((val, index) => <Dropdown.Item key={`top_discharge_port_${index}`} title={val.PORT_DESC} eventKey={index}
                                            active={index === loadingPort}>{val.ZGC_PORT}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
        ...
        <Col className="p-0" xs={6}>
            <Row noGutters>
                ...
                <Col className="p-0 " xs={4}>
                    <div className="top-tab row no-gutters justify-content-center text-secondary align-items-center w-100">
                        <Sorttab setHide={setHide} setOrderBy={setOrderBy} filter={filter} floors={floors} setFloors={setFloors} user={user} currentBu={currentBu} nav={nav} setTempFloors={setTempFloors} name="VALIDITY DATE" />
                    </div>
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                ...
                <Col xs={3}>
                    <Row noGutters>
                        ...
                        <Col className="p-0 " xs={6}>
                            ...
                            <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                                <div className="col-12 p-0 pr-1">
                                    <label className="bold m-0">FLOOR <span className="ml-1"><i onClick={() => sort("param2")} className="fas fa-sort"></i></span></label>
                                </div>

                            </div>
                            <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                                <div className="col-12 p-0 pr-1">
                                    <input type="text " value={floor} onChange={(e) => setFloor(e.target.value)} className="w-100  input-val" />
                                </div>
                ...
                <Col className="p-0" xs={3}>
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        <div className="col-5 p-0 pr-1">
                            <label className="bold m-0">CEILING <span className="ml-1"><i onClick={() => sort("param3")} className="fas fa-sort"></i></span></label>
                        </div>
                        <div className="col-7 p-0 pr-1 pl-1">
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        <div className="col-5 p-0 pr-1">
                            <input type="text " value={cieling} onChange={(e) => setCieling(e.target.value)} className="w-100  input-val" />
                        </div>
