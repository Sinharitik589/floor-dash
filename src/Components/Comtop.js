...
const Comtop = ({ selectIndexes, floors, setFloors, nav, user, currentBu }) => {
    ...
    const commodity_array = [
        {
            COMM_SUB_GROUP: "000000000001",
            TEXT: "LIVE ANIMALS"
        },
        {
            COMM_SUB_GROUP: "000000000002",
            TEXT: "MEAT & EDIBLE MEAT OFFAL"
        },
        {
            COMM_SUB_GROUP: "000000000003",
            TEXT: "FISH & CRUSTACEANS"
        },
        {
            COMM_SUB_GROUP: "000000000004",
            TEXT: "DAIRY, EGGS, HONEY, & ED. PRODUCTS"
        },
        {
            COMM_SUB_GROUP: "000000000005",
            TEXT: "PRODUCTS OF ANIMAL ORIGIN"
        },
        {
            COMM_SUB_GROUP: "000000000006",
            TEXT: "LIVE TREES & OTHER PLANTS"
        },
        {
            COMM_SUB_GROUP: "000000000007",
            TEXT: "EDIBLE VEGETABLES"
        },
        {
            COMM_SUB_GROUP: "000000000008",
            TEXT: "ED. FRUITS & NUTS, PEEL OF CITRUS/MELONS"
        },
        {
            COMM_SUB_GROUP: "000000000009",
            TEXT: "COFFEE, TEA, MATE & SPICES"
        },
        {
            COMM_SUB_GROUP: "000000000010",
            TEXT: "CEREALS"
        },
        {
            COMM_SUB_GROUP: "000000000011",
            TEXT: "MILLING INDUSTRY PRODUCTS"
        },
        {
            COMM_SUB_GROUP: "000000000012",
            TEXT: "OIL SEEDS/MISC. GRAINS/MED. PLANTS/STRAW"
        },
        {
            COMM_SUB_GROUP: "000000000013",
            TEXT: "LAC, GUMS, RESINS, ETC."
        },
        {
            COMM_SUB_GROUP: "000000000014",
            TEXT: "VEGETABLE PLAITING MATERIALS"
        },
        {
            COMM_SUB_GROUP: "000000000015",
            TEXT: "ANIMAL OR VEGETABLE FATS, OILS & WAXES"
        },
        {
            COMM_SUB_GROUP: "000000000016",
            TEXT: "ED. PREP. OF MEAT, FISH, CRUSTACEANS, ET"
        },
        {
            COMM_SUB_GROUP: "000000000017",
            TEXT: "SUGARS & SUGAR CONFECTIONERY"
        }
    ]
    ...
    let [comGroup, setComGroup] = useState("");
    ...
    const fetchFilterResult = async () => {
        ...
        if (charge !== "") params.param2 = charge;
        if (charge === "") delete params.param2;
        if (comGroup !== "") params.param3 = comGroup;
        if (comGroup === "") delete params.param3;
    ...
    useEffect(() => {
        fetchFilterResult()
    }, [loadingArea, dischargeArea, loadingCountry, dischargeCountry, loadingPort, dischargePort, pol, pod, cargo, charge, comGroup, start, end, trade, denomination])
                        ...
                        <Dropdown.Menu>
                            {
                                trade_array.length > 0 && trade_array.map((val, index) => {
                                    return <Dropdown.Item key={`top_trade_${index}`} title={val.TXTLG} eventKey={index} active={index === trade}>{val.ZCR_TRGR}</Dropdown.Item>
                                })
                            }
                        </Dropdown.Menu>
                    </Dropdown>
        ...
        <Col className="p-0 pr-1" xs={1}>
            ...
            <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                <div className="col-12 p-0 pr-1">
                    <label className="bold m-0">COMM. GROUP <span className="ml-1"><i onClick={() => sort("param3")} className="fas fa-sort"></i></span></label>
                </div>
            ...
            <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                <div className="col-12 p-0 pr-1">
                    <Dropdown onSelect={(e) => {
                        setComGroup(commodity_array[e].COMM_SUB_GROUP);
                    }}>
                        <Dropdown.Toggle
                            onChange={(e) => {
                                setComGroup(e.target.value)
                            }}
                            value={comGroup}
                        ...
                        <Dropdown.Menu >
                            {
                                commodity_array.map((val, index) => <Dropdown.Item key={`top_comgroup_${index}`} title={val.TEXT} eventKey={index} active={index === loadingPort}>{val.COMM_SUB_GROUP}</Dropdown.Item>)
                            }
                        </Dropdown.Menu>
                    </Dropdown>
                </div>
        ...
        <Col xs={5} className="p-0">
            <Row noGutters>
                <Col className="p-0 " xs={6}>
                    <div className="top-tab row no-gutters justify-content-center text-secondary align-items-center w-100">
                        <Sorttab setHide={setHide} setOrderBy={setOrderBy} filter={filter} setFloors={setFloors} floors={floors} user={user} currentBu={currentBu} nav={nav} name="LOADING" />
                    </div>
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                                ...
                                <Dropdown.Menu >
                                    {
                                        loading_area_array.map((val, index) =>
                                            <Dropdown.Item key={`top_loading_area_${index}`} title={val.TXTMD} eventKey={index} active={index === loadingArea}>{val.AREA}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                        ...
                        <div className="col-3 p-0 pr-1">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        loading_country_array.map((val, index) => <Dropdown.Item key={`top_loading_country_${index}`} eventKey={index} title={val.TXTKG} active={index === loadingCountry}>{val.COUNTRY}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                        <div className="col-2  p-0">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        loading_port_array.map((val, index) => <Dropdown.Item key={`top_loading_port_${index}`} title={val.PORT_DESC} eventKey={index} active={index === loadingPort}>{val.ZGC_PORT}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                ...
                <Col className="p-0 " xs={6}>
                    <div className="top-tab row no-gutters justify-content-center text-secondary align-items-center w-100">
                        <Sorttab setHide={setHide} setOrderBy={setOrderBy} filter={filter} setFloors={setFloors} floors={floors} user={user} currentBu={currentBu} nav={nav} name="DISCHARGE" />
                    </div>
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        <div className="col-2 p-0 ">
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        ...
                        <div className="col-3 p-0 pr-1">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        discharge_country_array.map((val, index) => <Dropdown.Item key={`top_discharge_country_${index}`} eventKey={index} title={val.TXTKG} active={index === loadingCountry}>{val.COUNTRY}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                        <div className="col-2  p-0">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Menu >
                                    {
                                        discharge_port_array.map((val, index) => <Dropdown.Item key={`top_discharge_port_${index}`} title={val.PORT_DESC} eventKey={index} active={index === loadingPort}>{val.ZGC_PORT}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
        ...
        <Col xs={5} className="p-0">
            <Row noGutters>
                ...
                <Col className="p-0 pr-1" xs={4}>
                    <div className="top-tab row no-gutters justify-content-center text-secondary align-items-center w-100">
                        <Sorttab setHide={setHide} setOrderBy={setOrderBy} filter={filter} floors={floors} setFloors={setFloors} user={user} currentBu={currentBu} nav={nav} name="VALIDITY DATE" />
                    </div>
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                    ...
                    <div className="top-option-tab row no-gutters p-1  align-items-center w-100 h-max-content">
                        <div className="col-12 p-0 pr-1">
                            <Dropdown onSelect={(e) => {
                                ...
                                <Dropdown.Toggle className={` row no-gutters p-1 align-items-center input-val bg-white ${(denomination === -1) ? "justify-content-end" : "justify-content-center "}`} as={"div"} >
                                    {denomination_array[denomination]}
                                </Dropdown.Toggle>

                                <Dropdown.Menu >
                                    {
                                        denomination_array.map((val, index) => <Dropdown.Item key={`top_discharge_deno_${index}`} eventKey={index} active={index === denomination}>{val}</Dropdown.Item>)
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
